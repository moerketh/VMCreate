...
namespace VMCreate.Gallery
{
    public class LoadParrotHome : IGalleryLoader
    {
        ...
        public async Task<List<GalleryItem>> LoadGalleryItems()
        {
            ...
            var preferredPattern = @"<a href=""(Parrot-home-[\d\.]+_amd64\.vmdk\.xz)"">.*?</a>\s+(\d{2}-[A-Za-z]{3}-\d{4} \d{2}:\d{2})\s+(\d+)";
            var match = Regex.Match(htmlContent, preferredPattern, RegexOptions.Singleline);

            if (!match.Success)
            {
                var fallbackPattern = @"<a href=""(Parrot-home-[\d\.]+_amd64\.iso)"">.*?</a>\s+(\d{2}-[A-Za-z]{3}-\d{4} \d{2}:\d{2})\s+(\d+)";
            ...
            var versionPattern = @"Parrot-home-([\d\.]+)_amd64\.(vmdk\.xz|iso)";
            ...
            var galleryItem = new GalleryItem
            {
                ...
                Description = $"Parrot Home Edition, for daily use with a focus on privacy and productivity (version {version})",
