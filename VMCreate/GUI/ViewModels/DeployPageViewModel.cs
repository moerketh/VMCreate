...
namespace VMCreate
{
    ...
    public class DeployPageViewModel : ViewModelBase
    {
        ...
        private void BuildPhaseList(WizardData wizardData)
        {
            ...
            {
                // Windows images (e.g. FLARE VM) are configured post-boot via PowerShell Direct.
                // Only the distribution options apply — no Linux infra steps.
                var c = wizardData.Customizations;
                bool hasWindowsPostBoot = c?.DistributionOptions?.Any(o => o.IsEnabled) == true;
        ...
        private void AddPostBootSubSteps(VmCustomizations c)
        {
            ...
            Phases.Add(new DeploymentPhase(SubWaitForSsh, "Waiting for VM",
                "Waiting for the VM to accept remote management connections", SymbolRegular.PlugConnected24) { IndentLevel = 1, IsVisible = false });
            ...
            {
                foreach (var opt in c.DistributionOptions)
                {
                    if (opt.IsEnabled)
                    {
                        Phases.Add(new DeploymentPhase(DistOptionSubId(opt.Name), opt.Name,
        ...
        private int InsertPostBootSubStepsAt(int index, VmCustomizations c)
        {
            Phases.Insert(index++, new DeploymentPhase(SubAddTempNic, "Add Temporary NIC",
                "Adding temporary network adapter for SSH access", SymbolRegular.PlugConnected24) { IndentLevel = 1, IsVisible = false });
            Phases.Insert(index++, new DeploymentPhase(SubWaitForSsh, "Waiting for VM",
                "Waiting for the VM to accept remote management connections", SymbolRegular.PlugConnected24) { IndentLevel = 1, IsVisible = false });
            ...
            if (c?.DistributionOptions != null)
            {
                foreach (var opt in c.DistributionOptions)
                {
                    if (opt.IsEnabled)
                    {
                        Phases.Insert(index++, new DeploymentPhase(DistOptionSubId(opt.Name), opt.Name,
                            "Running distribution-specific tool", SymbolRegular.ArrowSync24) { IndentLevel = 1, IsVisible = false });
                    }
                }
            }
